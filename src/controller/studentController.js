...
const createStudent = async function (req, res) {
  try {
    ...
    if (!isvalidRequestBody(requestBody)) {
      return res.status(400).send({ status: false, message: "Request body cant't be empty" });
    ...
    if (!isVAlidNumber(marks)) {
      return res.status(400).send({ status: false, message: "Marks should only contain numbers" });
...
const getStudent = async (req, res) => {
  try {
    ...
    return res.status(200).send({ status: true, message: "Success", data: fetchData });
  } catch (error) {
