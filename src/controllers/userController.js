...
const createUser = async function (req, res) {
  try {
   ...
   if (!isEmpty(title)) {
    return res.status(400).send({status: false,message: "Title is Missing or does not have a valid input"})}
   else {
  ...
  if (existedphone)
    ...
    if (!isEmpty(email)) {
      return res.status(400).send({status: false, message: "email is compulsory"});
    }
...
let loginUser = async function (req, res) {
  try {
    ...
    if (!checkData){
      return res.status(400).send({status: false,message: "email or the password is not correct"});
    }
    let token = jwt.sign({
