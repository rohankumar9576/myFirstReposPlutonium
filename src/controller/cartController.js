...
const createCart = async function (req, res) {
  try {
    ...
    if (!existCart) {
      ...
      let createCart = await cartModel.create({ userId: userIdByParams, items: newCart, totalPrice: productPrice, totalItems: 1 });
    ...
    if (existCart) {
      ...
      if (userIdbyToken !== findUserFromCart.userId.toString())
        return res.status(403).send({ status: false, message: "not authorized to update this cart", });
      ...
      if (flag === 0) {
        ...
        const saveData = await cartModel.findOneAndUpdate({ userId: userIdByParams }, {
...
const updateCart = async function (req, res) {
  try {
    ...
    if (removeProduct == 0 || removeProduct == 1);
      ...
      return res.status(400).send({ status: false, message: "please set removeProduct either 1 or 0 ", });
    ...
    if (!cartFind)
      return res.status(404).send({ status: false, message: "No products found in the cart" });
    ...
    if (!userFind)
      return res.status(404).send({ status: false, message: "user not found for the given userId", });
    ...
    if (itemsArr.length === 0)
      return res.status(400).send({ status: false, message: "cart is empty nothing to delete" });
    ...
    const updatedCart = await cartModel
      .findOneAndUpdate({ _id: cartId }, { items: itemsArr, totalPrice: totalPrice, totalItems: totalItems }, { new: true })
      .populate({
        path: "items.productId",
        select: "title price productImage -_id ",
      });
