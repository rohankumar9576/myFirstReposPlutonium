  ...
  try {
    ...
    if (priceSort) {
      if (priceSort != 1 && priceSort != -1) {
        return res.status(400).send({ status: false, message: "Please enter price sort 1 or -1" });
      }
    }
...
const updateProduct = async function (req, res) {
  try {
    ...
    let { title, price, isFreeShipping, availableSizes, installments } = requestBody;
    ...
    let updatingData = await productModel.findByIdAndUpdate({ _id: productId }, { $set: requestBody }, { new: true });
